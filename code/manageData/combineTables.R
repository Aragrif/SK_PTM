...
#+ warning = FALSE, message = FALSE
subfolder <- 'D:/UBC/R_code/SKAnalysis/data/benefits' #here("data", "benefits")
...
numrows <- (ngroups+1)*3 + 2 # total number of row to read in (5 rows for each strategy and the example [Best guess, Lower, Upper, Confidence, and a Quality check column], plus 1 row for group names)
experts <- c(1:12) # vector of expert codes, should correspond to the same order as in 'files'
...
for (i in 2:length(experts)){ # else use length(files) if all estimates are available
  ...
  #temp <- read.csv(files[i], skip = skiplines, nrows = numrows)
  temp <- temp %>% 
    select(0:numcols) %>%
  ...
  #temp <- temp[-1,] 
  temp <- data.frame(temp, fix.empty.names = TRUE) #rename empty colnames
  temp <- rename(temp,'Col'=2)
  temp$Col <- stri_replace_all_regex(temp$Col,
...
for(j in 3) {
  for(k in 1:3){ #best, lowest, highest
    names(byexpert)[n] <- paste(toString(temp[k,2]))
    n = n+1
  } 
}
for(j in 6:nstrat+5){ #for each strategy; 
  for(k in 1:3){ #best_1, lowest_1, highest_1 etc
...
byexpert$`Ecological Group`[which(str_detect(byexpert$`Ecological Group`, "HEALTHY PRAIRIE LANDSCAPE")==1)] <- "Healthy Prairie Landscape"

byexpert <- byexpert[!grepl('EXAMPLE', byexpert$`Ecological Group`),]

#' Output results
write.csv(byexpert, file="./results/Estimates_combined.csv")
